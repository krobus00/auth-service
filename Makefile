...
pb/auth/mock/mock_auth_service_client.go:
	mockgen -destination=pb/auth/mock/mock_auth_service_client.go -package=mock ${PACKAGE_NAME}/pb/auth AuthServiceClient
...
ifeq (dev server, $(filter dev server,$(MAKECMDGOALS)))
	...
	air --build.cmd 'go build -ldflags "-s -w -X $(PACKAGE_NAME)/internal/config.serviceVersion=$(VERSION) -X $(PACKAGE_NAME)/internal/config.serviceName=$(SERVICE_NAME)" -o bin/auth-service main.go' --build.bin "./bin/auth-service $(launch_args)"
...
build:
	# build binary file
	go build -ldflags "-s -w -X $(PACKAGE_NAME)/internal/config.serviceVersion=$(VERSION) -X $(PACKAGE_NAME)/internal/config.serviceName=$(SERVICE_NAME)" -o ./bin/auth-service ./main.go
